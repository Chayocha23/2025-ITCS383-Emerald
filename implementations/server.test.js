...
// ─────────────────────────────────────────────────────────────
    // Features 2 & 3: Customer Support & Notifications
    // ─────────────────────────────────────────────────────────────
    describe('Features 2 & 3: Customer Support & Notifications', () => {

        describe('Customer Support Tickets', () => {
            it('POST /api/support/tickets - should reject if required fields are missing', async () => {
                const res = await request(app).post('/api/support/tickets').send({ userId: 1 });
                expect(res.statusCode).toBe(400);
                expect(res.body.error).toBe('All fields are required.');
            });

            it('POST /api/support/tickets - should successfully create a ticket', async () => {
                mockSql.mockResolvedValueOnce([]); // Mock successful insert
                const res = await request(app).post('/api/support/tickets')
                    .send({ userId: 1, category: 'payment', message: 'Help me' });
                expect(res.statusCode).toBe(201);
                expect(res.body.message).toBe('Ticket created successfully!');
            });

            it('GET /api/employee/tickets - should fetch pending tickets for employee', async () => {
                mockAsRole('employee');
                mockSql.mockResolvedValueOnce([{ id: 1, category: 'payment', message: 'Help me', status: 'pending' }]);
                const res = await request(app).get('/api/employee/tickets?userId=2');
                expect(res.statusCode).toBe(200);
                expect(res.body.tickets.length).toBe(1);
            });

            it('POST /api/support/tickets/:id/reply - should allow admin to reply', async () => {
                mockAsRole('employee');
                mockSql.mockResolvedValueOnce([]); // Mock update
                const res = await request(app).post('/api/support/tickets/1/reply')
                    .send({ userId: 2, reply: 'Fixed it!' });
                expect(res.statusCode).toBe(200);
                expect(res.body.message).toBe('Reply sent successfully');
            });

            it('GET /api/user/messages - should fetch all user messages', async () => {
                mockSql.mockResolvedValueOnce([{ id: 1, message: 'Help me' }]);
                const res = await request(app).get('/api/user/messages?userId=1');
                expect(res.statusCode).toBe(200);
                expect(res.body.messages.length).toBe(1);
            });

            it('POST /api/support/tickets/:id/read - should mark ticket as read', async () => {
                mockSql.mockResolvedValueOnce([]); // Mock update
                const res = await request(app).post('/api/support/tickets/1/read');
                expect(res.statusCode).toBe(200);
                expect(res.body.success).toBe(true);
            });

            it('DELETE /api/support/tickets/:id - should delete a ticket', async () => {
                mockSql.mockResolvedValueOnce([]); // Mock delete
        ...
        describe('Notifications & Confirmations', () => {
            it('POST /api/employee/bookings/:id/confirm - should confirm booking', async () => {
                mockSql.mockResolvedValueOnce([]); // Mock update
                const res = await request(app).post('/api/employee/bookings/1/confirm');
                expect(res.statusCode).toBe(200);
                expect(res.body.message).toContain('Booking confirmed');
            });

            it('GET /api/user/notifications - should return stats for employee', async () => {
                mockSql
                    .mockResolvedValueOnce([{ role: 'employee' }]) // User role check
                    .mockResolvedValueOnce([{ count: 2 }])         // unread tickets
                    .mockResolvedValueOnce([{ count: 5 }]);        // pending bookings

                const res = await request(app).get('/api/user/notifications?userId=2');
                expect(res.statusCode).toBe(200);
                expect(res.body.unreadMessages).toBe(2);
                expect(res.body.pendingActionBookings).toBe(5);
            });

            it('GET /api/user/notifications - should return stats for customer', async () => {
                mockSql
                    .mockResolvedValueOnce([{ role: 'customer' }]) // User role check
                    .mockResolvedValueOnce([{ count: 1 }])         // unread tickets
                    .mockResolvedValueOnce([{ count: 3 }]);        // confirmed bookings

                const res = await request(app).get('/api/user/notifications?userId=1');
                expect(res.statusCode).toBe(200);
                expect(res.body.unreadMessages).toBe(1);
                expect(res.body.newlyConfirmedBookings).toBe(3);
            });
        });
    });